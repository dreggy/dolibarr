 ...
 * @param		int			$max_depth		max number of submenus level to output
 * @return	void
 */
 function print_eldy_megasubmenu($db,&$tabMenu, $parentMenu, $max_depth)
 {
	global $user, $conf, $langs, $cssStylePrinted, $colorbackhmenu1, $menumanager; 
	$menuTmp = new Menu();
	...
	for($s = 0; $s < $subMenuCount; $s++ )
	{
		...
		$sub = $subMenu[$s];
		if ( $sub["level"] <= $max_depth OR $max_depth == 0){
			if (1 /*$sub["enabled"]*/){
